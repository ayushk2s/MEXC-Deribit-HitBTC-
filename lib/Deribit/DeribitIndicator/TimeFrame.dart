...
class DeribitIndicator4 extends StatefulWidget {
  final String? timeFrameSmall, timeFrameLarge;
  DeribitIndicator4({this.timeFrameSmall = '15', this.timeFrameLarge = '15'});

  @override
  _DeribitIndicator4State createState() => _DeribitIndicator4State();
}

class _DeribitIndicator4State extends State<DeribitIndicator4>{
  ...
  Future<void> fetchOHLCDataFromDeribit() async {
    try {
      ...
      final int limit = timeLimit.isNotEmpty ? int.parse(timeLimit) : 1500; // Fetch only the latest candle

      // Get the current time (end timestamp)
      final int endTime = DateTime.now().millisecondsSinceEpoch;

      // Get the time 1 hour ago (start timestamp)
      final int startTime = DateTime.now().subtract(Duration(minutes: 1500)).millisecondsSinceEpoch;
          ...
          child: candlesSmall.isEmpty
            ...
            children: [
              ...
              Expanded(
                ...
                child: Column(
                  children: [
                    Expanded(flex: 2, child: DeribitIndicator2()), // Main OHLC Chart
                  ],
                ),
              ),
            ],
