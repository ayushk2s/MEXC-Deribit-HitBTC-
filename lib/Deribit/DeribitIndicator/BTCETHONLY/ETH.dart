...
class ETHINDICATOR extends StatefulWidget {
  final String? timeFrameSmall, timeFrameLarge;
  ETHINDICATOR({this.timeFrameSmall = '1', this.timeFrameLarge = '15'});

  @override
  _ETHINDICATORState createState() => _ETHINDICATORState();
}

class _ETHINDICATORState extends State<ETHINDICATOR>{
  ...
  Future<void> fetchOHLCDataFromDeribit() async {
    try {
      ...
      final int startTime = DateTime.now().subtract(Duration(minutes: 800)).millisecondsSinceEpoch;
          ...
          if (mounted) {
            setState(() {
              ...
              if ([2, 5, 10, 15, 20, 25, 32, 40, 45, 50, 55].contains(now.second)) {
                ...
                print('smaSmall length ${smaValuesSmall.length} largeSma ${smaValuesSecondSmall.length} '
                    'sarLength  ${sarValuesSmall.length} candle ${candlesSmall.length}');
